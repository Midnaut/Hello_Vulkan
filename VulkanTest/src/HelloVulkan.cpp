...
int HelloVulkan::rateDeviceSuitability(VkPhysicalDevice device) {
...
void HelloVulkan::createInstance() {
	...
	for (int i = 0; i < static_cast<int>(reqExtensionCount); i++) {
...
VKAPI_ATTR VkBool32 VKAPI_CALL HelloVulkan::debugCallback(
...
void HelloVulkan::frameBufferResizeCallback(GLFWwindow* window, int width, int height) {
...
VkExtent2D HelloVulkan::chooseSwapExtent(const VkSurfaceCapabilitiesKHR& capabilities) {
	// the max() here is getting interfered with by the windows sdk max, so we need the weird extra parenthesis
...
void HelloVulkan::createDescriptorSetLayout() {
	...
	if (vkCreateDescriptorSetLayout(device, &layoutInfo, nullptr, &descriptorSetLayout) != VK_SUCCESS) {
		throw std::runtime_error("Failed to create a Descript Set Layout");
	}
...
void HelloVulkan::createDepthResources() {
	...
	createImage(swapChainExtent.width, swapChainExtent.height, 1 , msaaSamples, format, VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, depthImage, depthImageMemory);
...
void HelloVulkan::createTextureImage() {
	...
	if (!pixels) {
		throw std::runtime_error("Failed to load a Texture Image!");
	}
	...
	// transition the IMAGE to dst optimal, we are moving our data in here
	transitionImageLayout(textureImage, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, mipLevels);
	copyBufferToImage(stagingBuffer, textureImage, static_cast<uint32_t>(texWidth), static_cast<uint32_t>(texHeight));
	// transition the IMAGE to shader read only optimal - this is going to be done inside mipmap generation
...
void HelloVulkan::createTextureSampler() {
	...
	if (vkCreateSampler(device, &samplerInfo, nullptr, &textureSampler) != VK_SUCCESS) {
		throw std::runtime_error("Failed to create Texture sSampler!");
	}
}
...
void HelloVulkan::updateUniformBuffer(uint32_t currentImage) {
	...
	ubo.view = glm::lookAt(glm::vec3(2.0f, 2.0f, 2.0f), glm::vec3(0.0f, 0.0f, 0.0f), glm::vec3(0.0f, 0.0f, 1.0));
	ubo.proj = glm::perspective(glm::radians(45.0f), swapChainExtent.width / (float)swapChainExtent.height, 0.1f, 10.0f);
	// glm made for OpenGL, need to flip the Y
	ubo.proj[1][1] *= -1;
...
void HelloVulkan::createSyncObjects() {
	...
	// instnatiate this to signalled to get over the first frame wait stalling issue
	...
	for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
		if (vkCreateSemaphore(device, &semaphoreInfo, nullptr, &imageAvailableSemaphores[i]) != VK_SUCCESS ||
			...
			throw std::runtime_error("failed to create Synchronisation Objects for frame!");
		}
...
void HelloVulkan::drawFrame() {
	...
	// reset rence after work has been submitted to prevent a locked state of the fence waiting
	...
	if (result == VK_ERROR_OUT_OF_DATE_KHR || result == VK_SUBOPTIMAL_KHR || frameBufferResized) {
		...
		throw std::runtime_error("failed to Present SwapCahin Image!");
	}
